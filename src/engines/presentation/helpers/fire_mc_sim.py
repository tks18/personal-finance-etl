    ...
    for i in prange(n_rows):  # type: ignore
        ...
        for j in range(iterations):
            ...
            for m in range(1, max_months + 1):
                ...
                if wealth > peak_wealth: peak_wealth = wealth
            ...
            if hit_month != -1:
                ...
                if months_left > 0:
                    ...
                    for d in range(1, months_left + 1):
                        ...
                        if dec_wealth > peak_wealth: peak_wealth = dec_wealth
        ...
        if valid_decum > 0:
            ...
            if len(v_dd) > 0: out_max_drawdown_p50[i] = np.percentile(v_dd, 50)
            
            v_ls = lost_savings[~np.isnan(lost_savings)]
            if len(v_ls) > 0: out_lost_savings_ev[i] = np.mean(v_ls)
            
            v_pi = peak_infs[~np.isnan(peak_infs)]
            if len(v_pi) > 0: out_peak_inf_p50[i] = np.percentile(v_pi, 50)
            
            v_sorr = sorr_cagrs[~np.isnan(sorr_cagrs)]
            if len(v_sorr) > 0: out_sorr_cagr_p10[i] = np.percentile(v_sorr, 10)
            
            v_swr = avg_swrs[~np.isnan(avg_swrs)]
            if len(v_swr) > 0: out_avg_swr_p50[i] = np.percentile(v_swr, 50)
        else:
            prob_success[i] = 0.0

    return (
        out_p90, out_p50, out_p10, prob_success, out_nom_p50, 
        out_runway_p90, out_runway_p50, out_runway_p10,
        out_terminal_wealth_p50, out_terminal_wealth_p10,
        out_max_drawdown_p50, out_lost_savings_ev,
        out_peak_inf_p50, out_sorr_cagr_p10, out_avg_swr_p50
    )
...
def get_monte_carlo_fire_batch(rules, cma_real_return, cma_fat_tail):
    def monte_carlo_fire_batch(s: pl.Series, **kwargs) -> pl.Series:
        ...
        core_res = _run_mc_simulations_numba(
            pv, pmt, fv, burn_core, inf_rates, seed_ints, iterations, max_months,
            vol_r, mean_r, swr, current_age_months, target_lifespan_months,
            tm_flat, st_bull, st_bear, st_stag, hc_prob, hc_min, hc_max,
            gp_derisk, gp_rerisk, gp_base, gp_target, jump_prob_ann, jump_mag,
            gp_debt_ret, gp_debt_vol, gk_upper, gk_lower, gk_cut, gk_raise,
            inf_theta, inf_vol_ann, inf_max, sorr_months
        )
        
        total_res = _run_mc_simulations_numba(
            pv, pmt_total, fv_total, burn_total, inf_rates, seed_ints, iterations, max_months,
            vol_r, mean_r, swr, current_age_months, target_lifespan_months,
            tm_flat, st_bull, st_bear, st_stag, hc_prob, hc_min, hc_max,
            gp_derisk, gp_rerisk, gp_base, gp_target, jump_prob_ann, jump_mag,
            gp_debt_ret, gp_debt_vol, gk_upper, gk_lower, gk_cut, gk_raise,
        ...
        df_out = pl.DataFrame({
            ...
            "Target_FI_Future_Nominal_P50": np.where(np.isnan(core_res[4]) | (core_res[4] == 0), np.nan, core_res[4]),
            "Target_FI_Total_Future_Nominal_P50": np.where(np.isnan(total_res[4]) | (total_res[4] == 0), np.nan, total_res[4]),
