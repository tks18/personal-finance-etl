...
def _run_mc_simulations_numba(
    ...
    for i in prange(n_rows):  # type: ignore
        ...
        if fv_c <= pv:
            out_p90_c[i], out_p50_c[i], out_p10_c[i] = 0.0, 0.0, 0.0
            prob_success_c[i] = 1.0
        
        if fv_t <= pv:
            out_p90_t[i], out_p50_t[i], out_p10_t[i] = 0.0, 0.0, 0.0
            prob_success_t[i] = 1.0
        ...
        # Estimate naive FI months
        est_fi_c = 0
        w = pv
        while w < fv_c and est_fi_c < max_months:
            w = w * (1.0 + mean_r) + pmt_c
            est_fi_c += 1

        est_fi_t = 0
        w = pv
        while w < fv_t and est_fi_t < max_months:
            w = w * (1.0 + mean_r) + pmt_t
            est_fi_t += 1
        ...
        for j in range(iterations):
            ...
            for m in range(1, max_runway_possible + 1):
                ...
                if m == 1 or (m - 1) % 12 == 0:
                    ...
                    if u_trans < p0: current_state = 0
                    elif u_trans < p0 + p1: current_state = 1
                    else: current_state = 2

                s_drift_ann = state_params[current_state, 0] - expense_drag - (jump_prob_ann * jump_mag)
                ...
                if inf_path < inf_min: inf_path = inf_min
                elif inf_path > inf_max: inf_path = inf_max
                if inf_path > path_peak_inf: path_peak_inf = inf_path
                
                cum_inf *= (1.0 + inf_path) ** (1.0 / 12.0)

                z_ret = np.random.standard_t(4)
                ...
                if hit_m_c == -1 and pv < fv_c:
                    ...
                    p_vol = np.sqrt((s_vol * eq_w)**2 + ((gp_debt_vol / np.sqrt(12.0)) * (1.0 - eq_w))**2)
                    
                    path_ls_c = path_ls_c * (1.0 + mean_r)
                    eff_pmt_c = -burn_c * (1.0 - hc_cov) if is_unemployed else pmt_c
                    if is_unemployed: path_ls_c += (pmt_c + burn_c * (1.0 - hc_cov))

                    ret = z_ret * (p_vol / np.sqrt(2.0)) + p_drift
                    eff_jump = jump * eq_w
                    w_c = w_c * (1.0 + ret) * (1.0 + eff_jump) + eff_pmt_c

                    if w_c > peak_w_c: peak_w_c = w_c
                    dd = (peak_w_c - w_c) / peak_w_c
                    if dd > max_dd_c: max_dd_c = dd
                    ...
                    if surv_c:
                        ...
                        swr_sum_c += (curr_wd_c * 12.0) / max(dec_w_c, 1.0)
                        ret = z_ret * (p_vol / np.sqrt(2.0)) + p_drift
                        eff_jump = jump * eq_w
                        ...
                        if dec_w_c > peak_w_c: peak_w_c = dec_w_c
                ...
                if hit_m_t == -1 and pv < fv_t:
                    ...
                    if is_unemployed: path_ls_t += (pmt_t + burn_t * (1.0 - hc_cov))

                    ret = z_ret * (p_vol / np.sqrt(2.0)) + p_drift
                    eff_jump = jump * eq_w
                    w_t = w_t * (1.0 + ret) * (1.0 + eff_jump) + eff_pmt_t

                    if w_t > peak_w_t: peak_w_t = w_t
                    dd = (peak_w_t - w_t) / peak_w_t
                    if dd > max_dd_t: max_dd_t = dd
                    ...
                    if surv_t:
                        ...
                        swr_sum_t += (curr_wd_t * 12.0) / max(dec_w_t, 1.0)
                        ret = z_ret * (p_vol / np.sqrt(2.0)) + p_drift
                        eff_jump = jump * eq_w
                        ...
                        if dec_w_t > peak_w_t: peak_w_t = dec_w_t
            ...
            if hit_m_c != -1 and hit_m_c <= max_months:
                ...
                if months_decum_c > 0:
                    swrs_c[j] = swr_sum_c / surv_m_c if surv_m_c > 0 else 0.0
            ...
            if hit_m_t != -1 and hit_m_t <= max_months:
                ...
                if months_decum_t > 0:
                    swrs_t[j] = swr_sum_t / surv_m_t if surv_m_t > 0 else 0.0
            ...
            if alv_c or alv_t:
                for d in range(1, max_runway_possible + 1):
                    ...
                    if alv_c:
                        r_w_c = r_w_c * (1.0 + ret) * (1.0 + jump) - burn_c
                        if r_w_c <= 0.0:
                            r_m_c = d
                            alv_c = False
                            
                    if alv_t:
                        r_w_t = r_w_t * (1.0 + ret) * (1.0 + jump) - burn_t
                        if r_w_t <= 0.0:
                            r_m_t = d
                            alv_t = False
                            
                    if not alv_c and not alv_t:
                        break
    ...
    return (
        out_p90_c, out_p50_c, out_p10_c, prob_success_c, out_nom_p50_c, out_runway_p90_c, out_runway_p50_c, out_runway_p10_c,
        out_terminal_wealth_p50_c, out_terminal_wealth_p10_c, out_max_drawdown_p50_c, out_lost_savings_ev_c, out_peak_inf_p50_c, out_sorr_cagr_p10_c, out_avg_swr_p50_c, out_terminal_wealth_nom_p50_c,
        out_p90_t, out_p50_t, out_p10_t, prob_success_t, out_nom_p50_t, out_runway_p90_t, out_runway_p50_t, out_runway_p10_t,
        out_terminal_wealth_p50_t, out_terminal_wealth_p10_t, out_max_drawdown_p50_t, out_lost_savings_ev_t, out_peak_inf_p50_t, out_sorr_cagr_p10_t, out_avg_swr_p50_t, out_terminal_wealth_nom_p50_t
    )
...
def get_monte_carlo_fire_batch(
    ...
    def monte_carlo_fire_batch(s: pl.Series, **kwargs: Any) -> pl.Series:
        ...
        mean_r = (1.0 + cma_real_return - expense_drag - (jump_prob_ann * jump_mag)) ** (1.0 / 12.0) - 1.0
        ...
        res = _run_mc_simulations_numba(
            pv, pmt_c, fv_c, burn_c, pmt_t, fv_t, burn_t,
            inf_rates, seed_ints, iterations, max_months, vol_r, mean_r, swr,
            current_age_months, target_lifespan_months, tm_flat, st_bull, st_bear, st_stag,
            hc_prob, hc_min, hc_max, hc_cov, gp_derisk, gp_rerisk, gp_base, gp_target, gp_dyn_debt,
            jump_prob_ann, jump_mag, gp_debt_ret, gp_debt_vol, gk_upper, gk_lower, gk_cut, gk_raise,
            inf_theta, inf_vol_ann, inf_max, inf_min, sorr_months, expense_drag
        )
        ...
        df_out = pl.DataFrame(
            {
                ...
                "Target_FI_Future_Nominal_P50": np.where(np.isnan(res[4]) | (res[4] == 0), np.nan, res[4]),
                "Target_FI_Total_Future_Nominal_P50": np.where(np.isnan(res[20]) | (res[20] == 0), np.nan, res[20]),
