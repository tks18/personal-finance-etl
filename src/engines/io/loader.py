...
def parse_date_col(df: pl.DataFrame) -> pl.DataFrame:
    ...
    if "Date" in df.columns and getattr(df.schema["Date"], "base_type", df.schema["Date"]) in [pl.Utf8, pl.String]:
        try:
            return df.with_columns(pl.col("Date").str.strptime(pl.Date, format="%Y-%m-%d", strict=False))
        except Exception:
...
class TaxDataLoader:
    ...
    def load_all(
        ...
        df_i = pl.read_csv(
            i_path, infer_schema_length=500_000) if i_path else None
        isin_master = {
            row["ISIN"]: row for row in df_i.to_dicts()} if df_i is not None else {}

        df_b = None
        if b_path and os.path.exists(b_path):
            df_b = parse_date_col(pl.read_csv(
                b_path, infer_schema_length=500_000))
    ...
    def load_from_dataframes(
        df_p: pl.DataFrame, df_s: pl.DataFrame, df_m: pl.DataFrame,
        df_i: pl.DataFrame | None, df_b: pl.DataFrame | None
    ) -> tuple[pl.DataFrame, pl.DataFrame, pl.DataFrame, dict, pl.DataFrame | None]:
