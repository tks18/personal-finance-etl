...
from personal_finance_etl.backend.utils.models import EngineStatus, ExportMode, LogLevel
...
class BaseEngineTab(ctk.CTkFrame):
    ...
    def handle_status(self, run_btn: ctk.CTkButton, status: EngineStatus) -> None:
        """Handle status updates from the backend engine. Thread-safe wrapper."""
        # Use self.after to ensure UI updates happen on the main Tkinter thread
        self.after(0, lambda: self._process_status(run_btn, status))

    def _process_status(self, run_btn: ctk.CTkButton, status: EngineStatus) -> None:
