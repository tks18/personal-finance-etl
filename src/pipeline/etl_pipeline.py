...
from src.utils.models import EngineStatus, LogLevel, ExtractionResult
from src.utils.interfaces import ILogger, IDatabaseLoader
...
from src.pipeline.strategies import AssetPipeline, StockPipeline, MutualFundPipeline
...
from src.load.database import (
    setup_sqlite_schema, generate_target_db_path, SQLiteLoader
)
...
class DataExtractor:
    ...
    def run(self) -> ExtractionResult:
        self.status_queue.put(EngineStatus(msg="Validating input configuration files...", data=None, progress=0.01, level=LogLevel.STEP))
        required_files = [
            self.cfg.COLUMN_MASTER_PATH, self.cfg.MF_ISIN_CSV_PATH, self.cfg.BENCHMARK_MAPPING_CSV_PATH, 
            self.cfg.BENCHMARK_MASTER_CSV_PATH, self.cfg.TAX_RATES_CSV_PATH, self.cfg.OPENING_BALANCE_CSV_PATH
        ]
        ...
        zcategory_lazy, assetgroup_lazy, assets_lazy, currency_lazy, inoutcome_lazy = extractor.extract_base_tables()
        ...
        return ExtractionResult(
            zcategory=zcategory_lazy, assetgroup=assetgroup_lazy, assets=assets_lazy,
            currency=currency_lazy, inoutcome=inoutcome_lazy, mappings=mappings,
...
class TransformationDAG:
    ...
    def run(self, extracted: ExtractionResult) -> dict[str, pl.DataFrame]:
        ...
        d_income_category_lazy = transform_d_income_category(extracted.zcategory, mappings["category"])
        d_income_subcategory_lazy = transform_d_income_subcategory(extracted.zcategory, mappings["category"], d_income_category_lazy)
        d_expense_category_lazy = transform_d_expense_category(extracted.zcategory, mappings["category"])
        d_expense_subcategory_lazy = transform_d_expense_subcategory(extracted.zcategory, mappings["category"])
        d_asset_category_lazy = transform_d_asset_category(extracted.assetgroup, mappings["asset_group"])
        d_asset_subcategory_lazy = transform_d_asset_subcategory(extracted.assets, mappings["assets"])
        d_currency_lazy = transform_d_currency(extracted.currency, mappings["currency"])
        d_benchmark_master_lazy = transform_d_investment_benchmark_master(self.cfg.BENCHMARK_MASTER_CSV_PATH)
        ...
        f_transfer_transactions_lazy = transform_f_transfer_transactions(base_transactions_lazy, d_asset_subcategory_lazy, d_asset_category_lazy)
        f_opening_balances_lazy = transform_f_opening_balances(self.cfg.OPENING_BALANCE_CSV_PATH, mappings["opbal"])
        ...
        market_data_lazy_list = [res.market_data for res in asset_results]
        ...
        stg_investment_market_data_lazy = transform_stg_investment_market_data(market_data_ref_lazy_list)
        ...
        d_tf_investment_master_lazy = get_d_tf_investment_master(master_ref_lazy_list, extracted.stg_benchmark_mapping)
        ...
        self.status_queue.put(EngineStatus(msg="Executing Base Transformation DAG in Parallel...", data=None, progress=0.2, level=LogLevel.STEP))
        ...
        return {
            "df_d_income_category": results[0], "df_d_income_subcategory": results[1], 
            "df_d_expense_category": results[2], "df_d_expense_subcategory": results[3],
            "df_d_asset_category": results[4], "df_d_asset_subcategory": results[5], 
            "df_d_currency": results[6], "df_d_benchmark_master": results[7], "df_d_tax_rates": results[8],
            "df_f_income_transactions": results[9], "df_f_expense_transactions": results[10], 
            "df_f_transfer_transactions": results[11], "df_f_opening_balances": results[12],
            "df_stg_investment_market_data": results[13], "df_f_tf_inv_purchase": results[14], 
            "df_f_tf_inv_sale": results[15], "df_d_tf_investment_master": results[16], "df_d_calendar": results[17]
        }
...
def run_pipeline(
    ...
    status_queue.put(EngineStatus(msg="Detecting date range for Benchmarks...", data=None, progress=0.35, level=LogLevel.STEP))
    ...
    min_market_date = market_dates.select(pl.min("Date")).item() if not market_dates.is_empty() else None
    max_market_date = market_dates.select(pl.max("Date")).item() if not market_dates.is_empty() else None
    min_purchase_date = purchase_dates.select(pl.min("Date")).item() if not purchase_dates.is_empty() else None
    ...
    # 4. Engine Processing
    status_queue.put(EngineStatus(msg="Starting Benchmark Engine...", data=None, progress=0.4, level=LogLevel.STEP))
    bm_engine = BenchmarkEngine(
        df_m=dfs["df_d_benchmark_master"], status_queue=status_queue, start_date=start_date, end_date=end_date,
        target_db_base_path=cfg.TARGET_DB_BASE_PATH, current_db_path=target_db_path
    )
    dfs["df_f_investment_benchmark_data"] = bm_engine.run()

    status_queue.put(EngineStatus(msg="Starting Polars Tax Engine...", data=None, progress=0.6, level=LogLevel.STEP))
    tax_engine = PolarsTaxEngine(
        df_p=dfs["df_f_tf_inv_purchase"], df_s=dfs["df_f_tf_inv_sale"],
        df_m=dfs["df_stg_investment_market_data"], df_i=dfs["df_d_tf_investment_master"],
        df_b=dfs["df_f_investment_benchmark_data"] if dfs["df_f_investment_benchmark_data"] is not None else pl.DataFrame(),
        df_t=dfs["df_d_tax_rates"], status_queue=status_queue, start_date=None, end_date=None
    )
