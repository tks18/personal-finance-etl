...
class UnifiedETLTab(BaseEngineTab):
    ...
    def _build_header(self) -> None:
        ...
        title_block.grid(row=0, column=0, columnspan=2,
                         padx=(16, 16), pady=(12, 0), sticky="w")
        ...
        ctk.CTkLabel(
            text_block, text="Shan's Personal Finance ETL",
            font=ctk.CTkFont(family="Segoe UI", size=15, weight="bold"),
            text_color=Color.TEXT,
        ).pack(anchor="w")

        ctk.CTkLabel(
            text_block, text="Automated Pipeline Orchestrator & Tax Engine",
        ...
        cfg_frame.grid(row=1, column=0, padx=(
            16, 10), pady=(12, 4), sticky="ew")
        ...
        cfg_entry = ctk.CTkComboBox(
            cfg_frame, variable=self.config_path_var, values=recents if recents else [
                ""],
            height=32, font=ctk.CTkFont(family="Segoe UI", size=12),
            fg_color="#080C14", border_color=Color.BORDER,
            border_width=1, corner_radius=5, command=self._on_config_selected
        )
        cfg_entry.grid(row=0, column=0, sticky="ew", padx=(0, 6))

        ctk.CTkButton(
            cfg_frame, text="…", width=32, height=32,
            font=ctk.CTkFont(size=14),
            fg_color="transparent", hover_color=Color.BORDER,
            border_width=1, border_color=Color.BORDER,
            corner_radius=4, command=lambda: self._select_file(self.config_path_var),
        ).grid(row=0, column=1)

        # ── 3. Run Button (Row 1, Col 1) (1/4th) ────────────────────
        btn_frame = ctk.CTkFrame(hdr, fg_color="transparent")
        btn_frame.grid(row=1, column=1, padx=(
            0, 16), pady=(12, 4), sticky="ew")
        btn_frame.grid_columnconfigure(0, weight=1)

        self.run_btn = ctk.CTkButton(
            btn_frame, text=self._run_btn_text,
            height=32,
            font=ctk.CTkFont(family="Segoe UI", size=13, weight="bold"),
            fg_color=Color.ACCENT, hover_color=Color.ACC_HOV,
            corner_radius=6, command=self._start_pipeline,
        )
        self.run_btn.grid(row=0, column=0, sticky="ew")

        # ── 4. Helper Info Box (Row 2) ──────────────────────────────
        info_block = ctk.CTkFrame(hdr, fg_color="transparent")
        info_block.grid(row=2, column=0, columnspan=2,
                        padx=(16, 16), pady=(4, 16), sticky="ew")
        ...
        ctk.CTkLabel(
            ...
            font=ctk.CTkFont(family="Segoe UI", size=10),
            text_color=Color.DIM, justify="left", anchor="w",
